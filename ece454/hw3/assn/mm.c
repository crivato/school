...
printf("PrevBlock %p\n", PREV_BLKP(bp));
	...
	if (GET(NEXT_BLKP(bp)+DSIZE) == 1010101010)
...
size = GET_SIZE(HDRP(bp));
	...
	else{ add_to_end_ll(bp);}/*
	else if( (size_t)UHead == GET(UTail)) //first free element
	{
		PUT(UHead, (size_t)bp);
		PUT(UTail, (size_t)NEXT(bp));
...
printf("find\n");
	...
	printf("utail %p\n", UTail );
	if(UHead!= NULL){
		printf("uhead holds %p\n", GET(UHead) );
		printf("utail holds %p\n", GET(UTail) );
	}
}
...
//printf("haiii\n");
		...
		PUT( HDRP(NEXT_BLKP(bp)), PACK((size_t)csize, 0) ); // make new header of following unallocated space
		PUT( FTRP(NEXT_BLKP(bp)), PACK((size_t)csize, 0) );

		add_to_end_ll(NEXT_BLKP(bp));
		return;
	}

	if( GET( bp+DSIZE ) == 1010101010 )
	{
	...
	else if(!GET_ALLOC(HDRP(bp)) )    {//putting at end of stack.  Don't care
...
printf("realloc gorw\n");
		...
		if (   !GET_ALLOC(HDRP(prevptr)) && difference <= GET_SIZE(HDRP(prevptr))   )  
		{

			if(!GET_ALLOC(HDRP(prevptr)) && GET( prevptr+DSIZE ) == 1010101010 )
				remove_from_ll(prevptr);

			size_t totalSize= ( GET_SIZE(HDRP(prevptr)) + oldsizeOH ); // contains buffer space
...
// Checks previous space if unallocated  AND  checks if enough new space exists    OR   Checks if previous AND next spaces are unallocated  AND if enough room between the two
		...
		{
if(DEBUG)
printf("realloc gorw 2\n");
			size_t totalSize = ( GET_SIZE(HDRP(nextptr)) + oldsizeOH ); // contains buffer space
			whiteSpace = totalSize - asize2; //contains overhead for buffers

if(DEBUG){
			printf("size %d asize2 %d\n", GET_SIZE( HDRP(oldptr)), asize2);
			printf("nxt%p\n",nextptr);
			printf("nxt nxt %p\n",GET(nextptr));
}
			if(!GET_ALLOC(HDRP(nextptr)) && GET( nextptr+DSIZE ) == 1010101010 )
